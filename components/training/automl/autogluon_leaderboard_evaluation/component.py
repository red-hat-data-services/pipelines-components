...
def leaderboard_evaluation(
    ...
    Args:
        ...
        component_status: Output artifact with stage progress (``component_status.json``).
            KFP creates this automatically when the component is used inside a pipeline.
    ...
    Example:
        ...
        def evaluation_pipeline(models_artifact):
            # component_status is wired by KFP; callers do not pass it explicitly.
    ...
    with status:
        ...
        for display_name in model_names:
            ...
            if not metrics_path.exists():
                logger.warning(
                    "Skipping model '%s': no metrics/metrics.json found. "
